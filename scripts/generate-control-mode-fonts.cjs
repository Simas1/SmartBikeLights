...
async function main() {
    ...
    for (const icon of ['headlight', 'taillight', 'night', 'flash']) {
    ...
    for (const [name, size] of Object.entries(fonts)) {
        ...
        const width = powerOfTwo((size + 1) * glyphs.length);
        const height = powerOfTwo(size);
        const images = [];
        const chars = [];
        for (const [index, [char, icon]] of fontGlyphs.entries()) {
            const svg = fs.readFileSync(path.join(app, 'assets', modeFont ? 'light-modes' : icon === 'power' ? 'power' : 'control-mode', icon + '.svg'), 'utf8')
                .replace(/#000000/g, '#FFFFFF');
            // Supersample before downscaling so tiny strokes retain coverage.
            const input = await sharp(Buffer.from(svg), {density: 768})
        ...
        const fnt = [
            ...
            `page id=0 file="${name}.png"`, `chars count=${glyphs.length}`, ...chars, ''
        ].join('\n');
